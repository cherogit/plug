...
const path = {
	...
	src: {
		pages: 'src/pug/*.pug',
		scripts: 'src/scripts/custom/script.js',
...
const processors = [
	...
	nested, // это должно быть до автопрефикса, иначе баг с фильтром и firefox
...
gulp.task('styles', function() {
	...
	return gulp.src(path.src.style)
		...
		.pipe(gulpIf(config.env === 'production', file('custom.css', '/*!*\n * Файл для дополнительных стилей: *\n */ \n\n\n.custom-style {\n\n}')))
...
gulp.task('scripts', function () {
	...
	return gulp.src(path.src.scripts)
		...
		.pipe(gulpIf(config.env === 'production', file('custom.js', '/*!*\n * Файл для дополнительных скриптов: *\n */ \n\n\n $(function() {\n \t// Файл для back-end разработчиков\n });')))
...
// gulp.task('createCustomFiles', function () {
...
gulp.task('build', gulp.series(
	'clean',
	gulp.parallel('styles:libs', 'styles', 'libsJS', 'scripts', 'fonts', 'images', 'source', 'jsonLint'), 'pug', 'typograf'
));
